:::demo `multiple-sort` 를 `true` 로 설정함으로서 복합 정렬을 할수 있습니다. <br>`sort-always` 를 설정하면 정렬을 오름차순 및 내림차순으로 만 변경할 수 있습니다.<br> `sort-change` 콜백 함수 를 전달하면 현재 정렬 된 필드가 얻어지고 정렬 `table-data` 는 목적을 달성하기 위해 정렬 결과가 다시 할당 됩니다.
```html
<template>
    <div>
        <v-table
        ...
        data(){
            return {
               ...
               columns: [
                    ...
                    {field: 'tel', title: '전화번호', width: 150, titleAlign: 'center',columnAlign:'center',orderBy:'desc',isResize:true},
